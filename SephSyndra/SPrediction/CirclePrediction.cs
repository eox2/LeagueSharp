    ...
    public static class CirclePrediction
    {
        ...
        public static Prediction.AoeResult GetAoePrediction(float width, float delay, float missileSpeed, float range, Vector2 from, Vector2 rangeCheckFrom)
        {
            ...
            if (enemies.Count() > 0)
            {
                ...
                {
                    var enemy = HeroManager.Enemies.Where(x => x.IsValidTarget(range)).MaxOrDefault(x => x.CountEnemiesInRange(width * 2));
                    var pred = GetPrediction(enemy, width, delay + flyTime, 0, range, false, enemy.GetWaypoints(), enemy.AvgMovChangeTime(), enemy.LastMovChangeTime(), enemy.AvgPathLenght(), enemy.LastAngleDiff(), from, rangeCheckFrom);
                    if (pred.HitChance >= HitChance.Medium)
                    {
                        result.CastPosition = pred.CastPosition;
                        result.HitCount = Math.Max(HeroManager.Enemies.Where(x => x.IsValidTarget() && x.Distance(pred.CastPosition) <= width).Count(), 1);
                    }
                }
                */
